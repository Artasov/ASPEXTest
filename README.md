...
- [Локальный запуск (без Docker Compose)](#локальный-запуск-без-docker-compose)
- [Запуск через Docker Compose](#запуск-через-docker-compose)
- [Grafana: как использовать](#grafana-как-использовать)
- [Тесты](#тесты)
...
### `ADMIN_EMAILS`
- Переменная окружения со списком email через запятую, например:
  - `ADMIN_EMAILS=admin@gmail.com,owner@company.com`
- Сравнение email выполняется в lower-case.
- При регистрации пользователь автоматически получает роль `admin`, если его email есть в этом списке.
